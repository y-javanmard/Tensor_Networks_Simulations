...
from tensor_networks_simulations.mps.algorithms import tebd_2nd_order_vidal_mpdo
...
def run(scale=1, gamma=0.05):
    ...
    Hs = [
        (1.0 / scale) * Hb.h_bond(i) + Hb.new_lindbladian(Hb.sx, i, gamma)
        for i in range(L - 1)
    ]
    # print(Hs)
    ...
    for i in range(total_steps + 1):
        rho, err = tebd_2nd_order_vidal_mpdo(
            rho=rho, Hs=Hs, chi_max=chi_max, dt_list=dt_list, epsilon=epsilon, d=4
        )
        # Ms = right_normalization_mps(rho.Ms, rho.bonds, d=4)
        # Ms, Ss = MPDO.schmidt_vals_from_mps(Ms, rho.bonds, d=4)
        # rho = MPDO(Ms, Ss, rho.bonds, d=4)
        if np.mod(i, int((20) * scale)) == (20 * scale) - 1:
            mz = sum([expectation_value(rho, Hb.sz, i).real for i in range(L)]) / L
            mx = sum([expectation_value(rho, Hb.sx, i).real for i in range(L)]) / L
            mxs.append(
                mx
                / (sum([expectation_value(rho, Hb.s0, i).real for i in range(L)]) / L)
            )
            mzs.append(
                mz
                / (sum([expectation_value(rho, Hb.s0, i).real for i in range(L)]) / L)
            ...
            ts.append((i + 1) * dt)
            tr_erros.append(err)
            print(f"mx={mx}, t={(i+1)*dt}/{scale*final_time}")
            print(f"norm=", expectation_value(rho, Hb.s0, 0).real)
            print(max(rho.bonds), scale)
            # check_right_normalization(rho.Ms)

            print("--------------------------")

    path = Path(f"data_{model}_{L}_{chi_max}")
    try:
    ...
    with open(
        f"data_{model}_{L}_{chi_max}/mxs_scale-{scale}_gamma-{gamma}.pickle", "wb"
    ) as fh:
        cPickle.dump(mxs, fh)
    with open(
        f"data_{model}_{L}_{chi_max}/mzs_scale-{scale}_gamma-{gamma}.pickle", "wb"
    ) as fh:
        cPickle.dump(mzs, fh)
    with open(
        f"data_{model}_{L}_{chi_max}/ts_scale-{scale}_gamma-{gamma}.pickle", "wb"
    ) as fh:
        cPickle.dump(ts, fh)
    ...
    if exact:
        ...
        expts = sf.integrate(L, hz, Jx, Jy, Jz, psi0, ts, gammas, solver)
...
if __name__ == "__main__":
    for scale in [0.25, 0.5, 0.75]:  # , 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]:
        for gamma in [
            0.001,
            0.0025,
            0.005,
            ...
            0.09,
            0.1,
        ]:  # [0.001, 0.0025, 0.005, 0.0075, 0.015, 0.02, 0.03 ]:#[0.01, 0.05, 0.1, 0.25, 0.5]:
            mzs, mxs, ts, mz_ex, mx_ex, tr_erros = run(scale, gamma)
            import matplotlib.pyplot as plt
            ...
            ax[0].plot(
                ...
                ms=4,
                ls="",
                label=rf"$<S_x>~ TEBD;\gamma={gamma},~ \alpha={scale} $",
            )
            ax[0].plot(
                ts,
                1 * np.array(mzs),
                ...
                ms=4,
                ls="",
                label=rf"$<S_z>~ TEBD; \gamma={gamma},~ \alpha={scale} $",
            )
